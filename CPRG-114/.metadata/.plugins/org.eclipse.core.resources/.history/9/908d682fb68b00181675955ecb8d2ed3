 ...
 *
 */
public final class ShoppingCartItemDAO extends AbstractDAO<ShoppingCartItemEntity> implements HasLogger {
    ...
    public List<CartItem> findDTOByCart(ShoppingCartEntity entity) {
        logger().trace("ENTER findDTOByCart(ShoppingCartEntity entity)");
        ...
        if (results.size() > 100_000)
            ...
            stream = results.stream();         
        logger().debug("results.stream().map(this::convert).collect(Collectors.toList()) {}", results.stream().map(this::convert).collect(Collectors.toList()));
        logger().trace("EXIT findDTOByCart(ShoppingCartEntity entity)");
