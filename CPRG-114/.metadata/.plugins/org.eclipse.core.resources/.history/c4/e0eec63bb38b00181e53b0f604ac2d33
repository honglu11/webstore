...
public final class ProductDAO extends AbstractDAO<ProductEntity> {
    ...
    public ProductEntity findEntityById(UUID id) {
        final TypedQuery<ProductEntity> query = 
                getEntityManager().createNamedQuery(ProductEntity.QUERY_FIND_BY_OID, ProductEntity.class);
        query.setParameter("oid", id);
    ...
    public Product convert(ProductEntity entity) {
        final Product dto = findDTO(entity.getOid());
