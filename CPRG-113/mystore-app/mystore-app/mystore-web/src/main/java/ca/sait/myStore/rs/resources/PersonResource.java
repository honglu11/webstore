...
public class PersonResource implements HasLogger {
    ...
    // @Path("/xml")
    @Path("{uuid}")
    public Person getPerson(@PathParam("uuid") String uuid) throws ResultNotFoundException, InvalidUUIDException {
    ...
    // @Path("/xml")
    @Path("email/{email}")
    public Person getPersonByEmail(@PathParam("email") String email) throws ResultNotFoundException {
        logger().trace("ENTER getPerson()");
        ...
        try {
            ...
            throw new ResultNotFoundException("Cannot find the email " + email);
        }
           finally {
            logger().trace("EXIT getPerson()");
        }
    ...
    @DELETE
    @Path("{uuid}")
    public Response deletePerson(@PathParam("uuid") String uuid, @Context UriInfo uriInfo) throws ResultNotFoundException, InvalidUUIDException {
    ...
    public Response createPerson(Person person, @Context UriInfo uriInfo) {
        logger().trace("ENTER createPerson()");
        logger().debug("Submit Person: {}", person.getEmail());
        try {
            final UriBuilder builder = uriInfo.getAbsolutePathBuilder();
            ...
            logger().trace("EXIT createPerson()");
        }
