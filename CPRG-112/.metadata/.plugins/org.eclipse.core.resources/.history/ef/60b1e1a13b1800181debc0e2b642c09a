...
public class ProductBackup implements Serializable {
    ...
    @AssertTrue(message="Description length must be between 100 and 2048 chars")
    private boolean validateDecription( ) {
        if (description == null || description.trim().isEmpty()) {
            return true;
        }
        
        if (description.length() < 100 || description.length() > 2048 ) {
            return false;            
        }
