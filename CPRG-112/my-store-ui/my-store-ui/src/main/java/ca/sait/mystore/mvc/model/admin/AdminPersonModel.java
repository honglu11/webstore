...
public class AdminPersonModel extends ViewModel {
    ...
    private void remove(HttpServletRequest request) {
        ...
        if (person.getOid() != null) {
            logger.debug("Remove OID: {}", person.getOid().toString());
            actionMessage = "Remove Person";
            entityManager.remove(person);
            persons.remove(person);
        } else {
            // don't need entityManager to update it since transaction take care it.
            logger.debug("person is not exisiting");
            actionMessage = "Cannot Remove None Existing Person";
