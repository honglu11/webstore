 ...
 */
@WebServlet(urlPatterns = { "/helloworld", "/hello", "/one/*" }, loadOnStartup=1)
public class HelloWorldServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
    private Logger logger = LoggerFactory.getLogger(getClass());
    private AtomicLong hitCounter = new AtomicLong();// thread safe
    private String appName;

    @Override
    public void init() throws ServletException {
        logger.trace("ENTER init()");
        appName = (String)getServletContext().getAttribute("appName"); // technically application can initial but cause lot of issue.
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            ...
            for (final Cookie reqCookie : request.getCookies()) {
                if (reqCookie.getName().equals("lifeCounter")) {
                    ...
                    lifeCounter = Long.parseLong(cookie.getValue());
                      ...
                      response.addCookie(new Cookie("niceTry", "LOL!"));
                    }
       ...
       // cookie.setPath(uri); evil to set Path, setSecure true mean only use https
        ...
        //final String appName = (String)request.getServletContext().getAttribute("appName");
